...
export default {
  ...
  data() {
    return {
      ...
      // 获取被选择项
      colSelected: ['Bindings', 'Age']
    }
  },
  computed: {
    clusterRoleBindingsInfo() {
      return this.$store.state.clusterRoleBindingsInfo.clusterRoleBindingsInfo
    },
    resourceData() {
      return this.clusterRoleBindingsInfo.filter(data => !this.search || data.name.toLowerCase().includes(this.search.toLowerCase()))
    },
    // 获取需要显示隐藏的表头列表
    tableTitle() {
      const arr = []
  ...
  methods: {
    fetchData() {
      this.$store.dispatch('clusterRoleBindingsInfo/getClusterRoleBindingsInfo')
    },
