        ...
        <template slot-scope="scope">
          <el-dropdown trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item icon="el-icon-edit-outline" @click.native="editServiceAccount(scope.row)">edit/view
              </el-dropdown-item>
              <el-dropdown-item icon="el-icon-delete" @click.native="deleteCurrentServiceAccount(scope.row)">delete
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
...
<script>
import { getServiceAccount, deleteServiceAccount } from '@/api/kubernetes/RBAC/serviceAccount'
...
export default {
  ...
  computed: {
    serviceAccountsInfo() {
      return this.$store.state.serviceAccountsInfo.serviceAccountsInfo
    },
    resourceData() {
      return this.serviceAccountsInfo.filter(data => !this.search || data.name.toLowerCase().includes(this.search.toLowerCase()))
    },
    currentNamespace() {
  ...
  methods: {
    fetchData() {
      this.$store.dispatch('serviceAccountsInfo/getServiceAccountsInfo', this.currentNamespace)
    },
    editServiceAccount(row) {
      getServiceAccount(row.namespace, row.name).then(res => {
        const tabData = {
          type: 'edit',
    ...
    deleteCurrentServiceAccount(row) {
      this.$confirm('即将从' + row.namespace + '命名空间删除' + row.name + ',是否继续？', this.$options.name, {
        ...
        new Promise((resolve, reject) => {
          deleteServiceAccount(row.namespace, row.name).then(response => {
            if (response.msg === 'ok') {
              this.$message({
                type: 'success',
                message: '删除成功'
