        ...
        <template slot-scope="scope">
          <el-dropdown trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item icon="el-icon-edit-outline" @click.native="editClusterRole(scope.row)">edit/view
              </el-dropdown-item>
              <el-dropdown-item icon="el-icon-delete" @click.native="deleteCurrentClusterRole(scope.row)">delete
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
...
<script>
import { getClusterRole, deleteClusterRole } from '@/api/kubernetes/RBAC/clusterRole'
...
export default {
  ...
  computed: {
    clusterRolesInfo() {
      return this.$store.state.clusterRolesInfo.clusterRolesInfo
    },
    resourceData() {
      return this.clusterRolesInfo.filter(data => !this.search || data.name.toLowerCase().includes(this.search.toLowerCase()))
    },
    // 获取需要显示隐藏的表头列表
    tableTitle() {
      const arr = []
  ...
  methods: {
    fetchData() {
      this.$store.dispatch('clusterRolesInfo/getClusterRolesInfo')
    },
    editClusterRole(row) {
      getClusterRole(row.name).then(res => {
        const tabData = {
          type: 'edit',
    ...
    deleteCurrentClusterRole(row) {
      this.$confirm('即将删除' + row.name + ',是否继续？', this.$options.name, {
        ...
        new Promise((resolve, reject) => {
          deleteClusterRole(row.name).then(response => {
            if (response.msg === 'ok') {
              this.$message({
                type: 'success',
                message: '删除成功'
