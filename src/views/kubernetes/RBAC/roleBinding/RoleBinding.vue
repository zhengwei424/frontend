<template>
  <div class="table-view">
    ...
    >
      ...
      />
      <el-table-column
        v-if="colOptions.bindings.isShow"
        sortable
        label="Bindings"
        prop="bindings"
      />
        ...
        <template slot-scope="scope">
          <el-dropdown trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item icon="el-icon-edit-outline" @click.native="editRoleBinding(scope.row)">edit/view
              </el-dropdown-item>
              <el-dropdown-item icon="el-icon-delete" @click.native="deleteCurrentRoleBinding(scope.row)">delete
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
...
<script>
import { getRoleBinding, deleteRoleBinding } from '@/api/kubernetes/RBAC/roleBinding'
...
export default {
  ...
  data() {
    return {
      ...
      // 获取被选择项
      colSelected: ['Namespace', 'Bindings', 'Age']
    }
  },
  computed: {
    roleBindingsInfo() {
      return this.$store.state.roleBindingsInfo.roleBindingsInfo
    },
    resourceData() {
      return this.roleBindingsInfo.filter(data => !this.search || data.name.toLowerCase().includes(this.search.toLowerCase()))
    },
    currentNamespace() {
  ...
  methods: {
    fetchData() {
      this.$store.dispatch('roleBindingsInfo/getRoleBindingsInfo', this.currentNamespace)
    },
    editRoleBinding(row) {
      getRoleBinding(row.namespace, row.name).then(res => {
        const tabData = {
          type: 'edit',
    ...
    deleteCurrentRoleBinding(row) {
      this.$confirm('即将从' + row.namespace + '命名空间删除' + row.name + ',是否继续？', this.$options.name, {
        ...
        new Promise((resolve, reject) => {
          deleteRoleBinding(row.namespace, row.name).then(response => {
            if (response.msg === 'ok') {
              this.$message({
                type: 'success',
                message: '删除成功'
