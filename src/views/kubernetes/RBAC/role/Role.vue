        ...
        <template slot-scope="scope">
          <el-dropdown trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item icon="el-icon-edit-outline" @click.native="editRole(scope.row)">edit/view
              </el-dropdown-item>
              <el-dropdown-item icon="el-icon-delete" @click.native="deleteCurrentRole(scope.row)">delete
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
...
<script>
import { getRole, deleteRole } from '@/api/kubernetes/RBAC/role'
...
export default {
  ...
  computed: {
    rolesInfo() {
      return this.$store.state.rolesInfo.rolesInfo
    },
    resourceData() {
      return this.rolesInfo.filter(data => !this.search || data.name.toLowerCase().includes(this.search.toLowerCase()))
    },
    currentNamespace() {
  ...
  methods: {
    fetchData() {
      this.$store.dispatch('rolesInfo/getRolesInfo', this.currentNamespace)
    },
    sendDataToDrawer(value) {
      console.log(value)
    },
    editRole(row) {
      getRole(row.namespace, row.name).then(res => {
        const tabData = {
          type: 'edit',
    ...
    deleteCurrentRole(row) {
      this.$confirm('即将从' + row.namespace + '命名空间删除' + row.name + ',是否继续？', this.$options.name, {
        ...
        new Promise((resolve, reject) => {
          deleteRole(row.namespace, row.name).then(response => {
            if (response.msg === 'ok') {
              this.$message({
                type: 'success',
                message: '删除成功'
