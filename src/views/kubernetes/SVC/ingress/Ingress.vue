        ...
        <template slot-scope="scope">
          <el-dropdown trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item icon="el-icon-edit-outline" @click.native="editIngress(scope.row)">edit/view
              </el-dropdown-item>
              <el-dropdown-item icon="el-icon-delete" @click.native="deleteCurrentIngress(scope.row)">delete
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
...
<script>
import { getIngress, deleteIngress } from '@/api/kubernetes/SVC/ingress'
...
export default {
  ...
  computed: {
    ingressesInfo() {
      return this.$store.state.ingressesInfo.ingressesInfo
    },
    resourceData() {
      return this.ingressesInfo.filter(data => !this.search || data.name.toLowerCase().includes(this.search.toLowerCase()))
    },
    currentNamespace() {
  ...
  methods: {
    fetchData() {
      this.$store.dispatch('ingressesInfo/getIngressesInfo', this.currentNamespace)
    },
    sendDataToDrawer(value) {
      console.log(value)
    },
    editIngress(row) {
      getIngress(row.namespace, row.name).then(res => {
        const tabData = {
          type: 'edit',
    ...
    deleteCurrentIngress(row) {
      this.$confirm('即将从' + row.namespace + '命名空间删除' + row.name + ',是否继续？', this.$options.name, {
        ...
        new Promise((resolve, reject) => {
          deleteIngress(row.namespace, row.name).then(response => {
            if (response.msg === 'ok') {
              this.$message({
                type: 'success',
                message: '删除成功'
