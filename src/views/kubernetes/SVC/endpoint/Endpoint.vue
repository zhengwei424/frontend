<template>
  <div class="table-view">
    ...
    >
      ...
      />
      <el-table-column
        v-if="colOptions.endpoints.isShow"
        sortable
        label="Endpoints"
      >
        <template slot-scope="scope">
          <div v-for="(v, i) in scope.row.endpoints" :key="i">{{ v }}</div>
        </template>
      </el-table-column>
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-dropdown trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item icon="el-icon-edit-outline" @click.native="editEndpoint(scope.row)">edit/view
              </el-dropdown-item>
              <el-dropdown-item icon="el-icon-delete" @click.native="deleteCurrentEndpoint(scope.row)">delete
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
...
<script>
import { deleteEndpoint, getEndpoint } from '@/api/kubernetes/SVC/endpoint'
...
export default {
  ...
  data() {
    return {
      ...
      // 获取被选择项
      colSelected: ['Namespace', 'Endpoints', 'Age']
    }
  },
  computed: {
    endpointsInfo() {
      return this.$store.state.endpointsInfo.endpointsInfo
    },
    resourceData() {
      return this.endpointsInfo.filter(data => !this.search || data.name.toLowerCase().includes(this.search.toLowerCase()))
    },
    currentNamespace() {
  ...
  methods: {
    fetchData() {
      this.$store.dispatch('endpointsInfo/getEndpointsInfo', this.currentNamespace)
    },
    editEndpoint(row) {
      getEndpoint(row.namespace, row.name).then(res => {
        const tabData = {
          type: 'edit',
    ...
    deleteCurrentEndpoint(row) {
      this.$confirm('即将从' + row.namespace + '命名空间删除' + row.name + ',是否继续？', this.$options.name, {
        ...
        new Promise((resolve, reject) => {
          deleteEndpoint(row.namespace, row.name).then(response => {
            if (response.msg === 'ok') {
              this.$message({
                type: 'success',
                message: '删除成功'
