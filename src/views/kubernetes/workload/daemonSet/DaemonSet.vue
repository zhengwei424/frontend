        ...
        <template slot-scope="scope">
          <el-dropdown trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item icon="el-icon-edit-outline" @click.native="editDaemonSet(scope.row)">edit/view
              </el-dropdown-item>
              <el-dropdown-item icon="el-icon-delete" @click.native="deleteCurrentDaemonSet(scope.row)">delete
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
...
<script>
import { getDaemonSet, deleteDaemonSet } from '@/api/kubernetes/workload/daemonSet'
...
export default {
  ...
  computed: {
    daemonSetsInfo() {
      return this.$store.state.daemonSetsInfo.daemonSetsInfo
    },
    resourceData() {
      return this.daemonSetsInfo.filter(data => !this.search || data.name.toLowerCase().includes(this.search.toLowerCase()))
    },
    currentNamespace() {
  ...
  methods: {
    fetchData() {
      this.$store.dispatch('daemonSetsInfo/getDaemonSetsInfo', this.currentNamespace)
    },
    sendDataToDrawer(value) {
      console.log(value)
    },
    editDaemonSet(row) {
      getDaemonSet(row.namespace, row.name).then(res => {
        const tabData = {
          type: 'edit',
    ...
    deleteCurrentDaemonSet(row) {
      this.$confirm('即将从' + row.namespace + '命名空间删除' + row.name + ',是否继续？', this.$options.name, {
        ...
        new Promise((resolve, reject) => {
          deleteDaemonSet(row.namespace, row.name).then(response => {
            if (response.msg === 'ok') {
              this.$message({
                type: 'success',
                message: '删除成功'
