<template>
  <div class="table-view">
    ...
    >
      ...
      />
      <el-table-column
        v-if="colOptions.schedule.isShow"
        sortable
        label="Schedule"
        prop="schedule"
      />
      <el-table-column
        v-if="colOptions.suspend.isShow"
        sortable
        label="Suspend"
        prop="suspend"
      />
      <el-table-column
        v-if="colOptions.active.isShow"
        sortable
        label="Active"
        prop="active"
      />
      <el-table-column
        v-if="colOptions.lastSchedule.isShow"
        sortable
        label="Last Schedule"
        prop="lastSchedule"
      />
        ...
        <template slot-scope="scope">
          <el-dropdown trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item icon="el-icon-edit-outline" @click.native="editCronJob(scope.row)">edit/view
              </el-dropdown-item>
              <el-dropdown-item icon="el-icon-delete" @click.native="deleteCurrentCronJob(scope.row)">delete
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
...
<script>
import { getCronJob, deleteCronJob } from '@/api/kubernetes/workload/cronJob'
...
export default {
  ...
  data() {
    return {
      ...
      // 获取被选择项
      colSelected: ['Namespace', 'Schedule', 'Suspend', 'Active', 'Last Schedule', 'Age']
    }
  },
  computed: {
    cronJobsInfo() {
  ...
  methods: {
    fetchData() {
      this.$store.dispatch('cronJobsInfo/getCronJobsInfo', this.currentNamespace)
    },
    editCronJob(row) {
      getCronJob(row.namespace, row.name).then(res => {
        const tabData = {
          type: 'edit',
    ...
    deleteCurrentCronJob(row) {
      this.$confirm('即将从' + row.namespace + '命名空间删除' + row.name + ',是否继续？', this.$options.name, {
        ...
        new Promise((resolve, reject) => {
          deleteCronJob(row.namespace, row.name).then(response => {
            if (response.msg === 'ok') {
              this.$message({
                type: 'success',
                message: '删除成功'
