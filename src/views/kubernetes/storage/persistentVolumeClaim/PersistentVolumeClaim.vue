        ...
        <template slot-scope="scope">
          <el-dropdown trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item icon="el-icon-edit-outline" @click.native="editPersistentVolumeClaim(scope.row)">edit/view
              </el-dropdown-item>
              <el-dropdown-item icon="el-icon-delete" @click.native="deleteCurrentPersistentVolumeClaim(scope.row)">delete
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
...
<script>
import { getPersistentVolumeClaim, deletePersistentVolumeClaim } from '@/api/kubernetes/storage/persistentVolumeClaim'
...
export default {
  ...
  methods: {
    fetchData() {
      this.$store.dispatch('persistentVolumeClaimsInfo/getPersistentVolumeClaimsInfo', this.currentNamespace)
    },
    editPersistentVolumeClaim(row) {
      getPersistentVolumeClaim(row.namespace, row.name).then(res => {
        const tabData = {
          type: 'edit',
    ...
    deleteCurrentPersistentVolumeClaim(row) {
      this.$confirm('即将从' + row.namespace + '命名空间删除' + row.name + ',是否继续？', this.$options.name, {
        ...
        new Promise((resolve, reject) => {
          deletePersistentVolumeClaim(row.namespace, row.name).then(response => {
            if (response.msg === 'ok') {
              this.$message({
                type: 'success',
                message: '删除成功'
