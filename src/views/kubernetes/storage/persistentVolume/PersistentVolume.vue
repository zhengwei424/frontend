        ...
        <template slot-scope="scope">
          <el-dropdown trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item icon="el-icon-edit-outline" @click.native="editPersistentVolume(scope.row)">edit/view
              </el-dropdown-item>
              <el-dropdown-item icon="el-icon-delete" @click.native="deleteCurrentPersistentVolume(scope.row)">delete
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
...
<script>
import { getPersistentVolume, deletePersistentVolume } from '@/api/kubernetes/storage/persistentVolume'
...
export default {
  ...
  computed: {
    persistentVolumesInfo() {
      return this.$store.state.persistentVolumesInfo.persistentVolumesInfo
    },
    resourceData() {
      return this.persistentVolumesInfo.filter(data => !this.search || data.name.toLowerCase().includes(this.search.toLowerCase()))
    },
    // 获取需要显示隐藏的表头列表
    tableTitle() {
      const arr = []
  ...
  methods: {
    ...
    deleteCurrentPersistentVolume(row) {
      this.$confirm('即将删除' + row.name + ',是否继续？', this.$options.name, {
        ...
        new Promise((resolve, reject) => {
          deletePersistentVolume(row.name).then(response => {
            if (response.msg === 'ok') {
              this.$message({
                type: 'success',
                message: '删除成功'
