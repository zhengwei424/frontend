        ...
        <template slot-scope="scope">
          <el-dropdown trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item icon="el-icon-edit-outline" @click.native="editStorageClass(scope.row)">edit/view
              </el-dropdown-item>
              <el-dropdown-item icon="el-icon-delete" @click.native="deleteCurrentStorageClass(scope.row)">delete
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
...
<script>
import { getStorageClass, deleteStorageClass } from '@/api/kubernetes/storage/storageClass'
...
export default {
  ...
  computed: {
    ...
    resourceData() {
      return this.storageClassesInfo.filter(data => !this.search || data.name.toLowerCase().includes(this.search.toLowerCase()))
    },
    // 获取需要显示隐藏的表头列表
    tableTitle() {
      const arr = []
  ...
  methods: {
    ...
    editStorageClass(row) {
      getStorageClass(row.name).then(res => {
        const tabData = {
          type: 'edit',
    ...
    deleteCurrentStorageClass(row) {
      this.$confirm('即将删除' + row.name + ',是否继续？', this.$options.name, {
        ...
        new Promise((resolve, reject) => {
          deleteStorageClass(row.name).then(response => {
            if (response.msg === 'ok') {
              this.$message({
                type: 'success',
                message: '删除成功'
